...
meg_ordination <- function(data_list,
    ...
    for( l in 1:length(local_obj) ) {
        ...
        if(is.null(dim(t_data))) {
            t_data = matrix(t_data, nrow=1)
        }
        t_data <- t_data[which(!is.na(local_meta[[sample_var]]) & local_meta[[sample_var]] != 'NA'), 
                         colSums(t_data) > 0 & apply(t_data, 2, function(x) abs(max(x) - min(x))) > 0]
